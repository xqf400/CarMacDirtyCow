    ...
    "Test that unaligned copy respects read-only mapping")
{
    ...
    fprintf(stderr, "maxSize %lu \n", PAGE_SIZE);
    [[NSNotificationCenter defaultCenter] postNotificationName: @"writeToConsole" object:nil userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"maxSize %lu",PAGE_SIZE] forKey:@"message"]];
    
    fprintf(stderr, "myySize %lu \n", overwrite_length);
    [[NSNotificationCenter defaultCenter] postNotificationName: @"writeToConsole" object:nil userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"myySize %lu",overwrite_length] forKey:@"message"]];

    
    if (overwrite_length >= PAGE_SIZE) {
        fprintf(stderr, "too long!\n");
        [[NSNotificationCenter defaultCenter] postNotificationName: @"writeToConsole" object:nil userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"too long!"] forKey:@"message"]];
        //exitApp();
    }
    ...
    if (overwrite_first_diff_offset == -1) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName: @"writeToConsole" object:nil userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"no diff?"] forKey:@"message"]];
    }
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName: @"writeToConsole" object:nil userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Testing for %ld seconds...", duration] forKey:@"message"]];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName: @"writeToConsole" object:nil userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"vm_read_overwrite: KERN_SUCCESS:%d KERN_PROTECTION_FAILURE:%d other:%d", kern_success, kern_protection_failure, kern_other] forKey:@"message"]];
    T_PASS("Ran %d times in %ld seconds with no failure", loops, duration);
    [[NSNotificationCenter defaultCenter] postNotificationName: @"writeToConsole" object:nil userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Ran %d times in %ld seconds with no failure", loops, duration] forKey:@"message"]];
}
...
void overwrite(NSData *imageData, NSString *path, NSString *name) {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName: @"writeToConsole" object:nil userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"filePath %@", filePath] forKey:@"message"]];
    
    NSLog(@"data size: %lu" , imageData.length);
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 5 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
            ...
            NSString *dirName = [docDir stringByAppendingPathComponent:name];
            const char *model_path = dirName.UTF8String;
            const char *target = path.UTF8String;
            //NSLog(@"model_path %s", model_path);
